...
- **Пути:** для systemd задайте `WorkingDirectory` на корень клона; для state вне cwd — абсолютный `HTR_STATE_FILE` и/или `HTR_PROJECT_ROOT`. Каталог под lock-файл должен существовать и принадлежать пользователю сервиса.
